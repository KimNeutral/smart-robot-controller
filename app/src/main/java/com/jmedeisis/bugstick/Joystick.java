...
public class Joystick extends FrameLayout {
    ...
    private static final int STICK_SETTLE_DURATION_MS = 0;
    ...
    private void recalculateRadius(int width, int height) {
        ...
        // NOTE: 개조한 부분, Radius 계산할 때 공의 위치 변경.
        if (hasStick()) {
            final View stick = getChildAt(0);
            stick.setTranslationX(0);
            stick.setTranslationY(-radius);
        }
    }
    ...
    private void onDragStop() {
        ...
        if (!locked) {
            draggedChild.animate()
                    .translationX(0).translationY(-radius)
...
//        if (x * x + y * y > radius * radius)
        ...
        // NOTE: 개조한 부분, 무조건 음수를 취함으로써 0~180도으로만 제어하도록.
        draggedChild.setTranslationY(y > 0 ? -y : y);
